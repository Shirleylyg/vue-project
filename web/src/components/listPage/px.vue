<template>
    <div class="pxComponent">
        <ul @click="active">
            <li v-for="(val,idx) in dataset" v-bind="lming">
        ...
        methods:{
            active:function(e){
                ...
                if(yuan.innerText==lowPrice){
                    axios.get('http://localhost:1200/lowPrice',{params:{date:"lowPrice"}}).then(response=>{
                ...
                if(yuan.innerText==highPrice){
                    axios.get('http://localhost:1200/highPrice',{params:{date:"highPrice"}}).then(response=>{
                        var dates=response.data.results;
                        this.dates=dates;
                        var all =document.querySelector('.pxComponent');
                        console.log(all);
                ...
                // if(kinds==lowPrice){
                // //排序 低价优先
                //     axios.get('http://localhost:1200/px',{params:{date:''}}).then(response=>{
                //         console.log(response);
                //     })
                ...
                if(yuan.innerText==goodPing){
                    axios.get('http://localhost:1200/goodPing',{params:{date:"goodPing"}}).then(response=>{
                        var dates=response.data.results;
                        this.dates=dates;
                        var all =document.querySelector('.pxComponent');
                        all.innerHTML=this.dates.map(function(item){
                ...
                if(yuan.innerText==render){
                    axios.get('http://localhost:1200/rander',{params:{date:"rander"}}).then(response=>{
                        var dates=response.data.results;
                        this.dates=dates;
                        var all =document.querySelector('.pxComponent');
                        all.innerHTML=this.dates.map(function(item){
